...
import com.fluig.service.CreateFolderService;
...
public class CreateFolderController {
    ...
    public Response execute(String jsonValues) {
        ...
        CreateFolderModel requestMessage = parseJsonToCreateFolderModel(jsonValues);
        ...
        try {
            responseMessage = new CreateFolderService().executeService(requestMessage);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private CreateFolderModel parseJsonToCreateFolderModel(String jsonValues) {
        try {
            return gson.fromJson(jsonValues, CreateFolderModel.class);
        } catch (JsonSyntaxException | IllegalArgumentException | UnsupportedOperationException | ConcurrentModificationException e) {
            return null;
        }
    }

    private boolean isValidRequestMessage(CreateFolderModel requestMessage) {
        return requestMessage.getDocumentDescription() != null && !requestMessage.getDocumentDescription().isEmpty() && requestMessage.getParentFolderId() != null && !requestMessage.getParentFolderId().isEmpty();
    }
